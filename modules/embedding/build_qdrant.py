...
from modules.embedding.embedder import get_bge_m3_embeddings
...
def main() -> None:
    ...
    # 3) bge-m3 임베딩
    embeddings = get_bge_m3_embeddings(device="cpu")  # 나중에 "mps"로 바꿔볼 수 있음

    # 4) Qdrant 로컬 저장 경로
    qdrant_path = Path(paths.outputs_dir) / "qdrant_db"
    collection_name = f"rfp_{mode}_bge_m3"
